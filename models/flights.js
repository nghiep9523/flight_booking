...
function Flights() {
    ...
    this.searchFlights = function(query, res) {
        connection.acquire(function(err, con) {
            con.query('select distinct chuyen_bay.Ma, chuyen_bay.Ngay, chuyen_bay.Gio, chuyen_bay.ThoiGianBay, chi_tiet_chuyen_bay.Hang, chi_tiet_chuyen_bay.MucGia, chi_tiet_chuyen_bay.GiaBan, thong_tin_ve.SoLuong, thong_tin_ve.MaVe from chuyen_bay inner join thong_tin_chuyen_bay on chuyen_bay.Ma = thong_tin_chuyen_bay.Ma inner join chi_tiet_chuyen_bay on chuyen_bay.Ma = chi_tiet_chuyen_bay.MaChuyenBay and chuyen_bay.Ngay = chi_tiet_chuyen_bay.Ngay inner join loai_ve on chuyen_bay.ThongTinVe = loai_ve.MaLoaiVe inner join thong_tin_ve on loai_ve.MaThongTin = thong_tin_ve.MaVe where thong_tin_chuyen_bay.NoiDi = ? and thong_tin_chuyen_bay.NoiDen = ? and chuyen_bay.Ngay = ? and thong_tin_ve.SoLuong >= ?', [query.from, query.to, query.date, query.amount], function(err, result) {
                con.release();
                res.send(result);
            });
        });
    ...
    connection.acquire(function(err, con) {
      console.log(request);
      con.query('UPDATE chuyen_bay SET Ma = ?, Ngay = ?, Gio = ?, ThoiGianBay = ?, ThongTinVe = ? WHERE Ma = ? and Ngay = ?',
     [request.MaCBMoi,  request.Ngay, request.Gio, request.ThoiGianBay, null, request.MaCB, request.NgayCu], function(err, result) {
        //con.release();
         if (err) {
          console.log(err);
          res.send({status: 1, message: 'Record update failed'});
        } else {
        con.query('UPDATE loai_ve SET MaLoaiVe = ? WHERE MaLoaiVe = ?',
        [request.MaCBMoi + request.Ngay, request.MaCB + request.NgayCu], function(err, result) {
        //con.release();
         console.log(request);
         if (err) {
          console.log(err);
          res.send({status: 1, message: 'Record update failed'});
           ...
           con.query('UPDATE chuyen_bay SET ThongTinVe = ? WHERE Ma = ? and Ngay = ?',
        [request.MaCBMoi + request.Ngay, request.MaCBMoi, request.Ngay], function(err, result) {
         con.release();
          console.log(request);
         if (err) {
          console.log(err);
